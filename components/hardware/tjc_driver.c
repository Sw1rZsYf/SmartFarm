...
//             if (length > 0)
//             {
//                 ESP_LOGW(TAG, "接收指令超时，已接收 %d 字节", length);
//             }
//             break;
//         }

//         // 读取一个字节（非阻塞）
//         int len = uart_read_bytes(TJC_UART_PORT_NUM, temp + length, 1, 20);
//         if (len > 0)
//         {
//             length += len;
//             // 检查是否收到完整的结束符 (三个0xFF)
...
//             {
//                 // 去掉结束符，拷贝指令内容
//                 size_t cmd_len = length - 3;
//                 if (cmd_len >= buf_size)
//                 {
//                     ESP_LOGE(TAG, "指令过长，缓冲区不足");
//                     return -2;
//                 }
//                 memcpy(buffer, temp, cmd_len);
//                 buffer[cmd_len] = '\0'; // 添加字符串结束符
//                 ESP_LOGD(TAG, "收到指令: %s", buffer);
//                 return cmd_len;
//             }
//             // 防止缓冲区溢出
//             if (length >= sizeof(temp) - 3)
//             {
//                 ESP_LOGE(TAG, "接收缓冲区溢出");
//                 // 丢弃数据，重新开始
//                 length = 0;
//             }
//         }
//         vTaskDelay(pdMS_TO_TICKS(10)); // 避免空转
//     }
//     return 0; // 无完整指令
// }

// void tjc_parse_command(const char *cmd)
// {
//     if (cmd == NULL)
//         return;
